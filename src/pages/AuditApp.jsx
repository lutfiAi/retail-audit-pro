...
function DeptCard({dept,scores,active,onClick,lang,editMode,onEdit,onDelete}){
  const pct=deptPct(dept,scores),answered=allItems(dept).filter(it=>it.id in scores).length,total=allItems(dept).length;
  return(
    <div className="relative shrink-0">
      <button onClick={onClick} className={`flex flex-col items-center gap-1.5 rounded-2xl px-4 py-3 border-2 transition-all duration-200 min-w-[130px] cursor-pointer
        ${editMode?"border-amber-300 pb-8":""}
        ${active?`${dept.color.bg} text-white border-transparent shadow-lg scale-105`:`bg-white ${editMode?"":""+dept.color.border} ${dept.color.text} hover:shadow-md`}`}>
      ...
      {editMode&&(
        <div className="absolute bottom-1.5 left-0 right-0 flex justify-center gap-1">
          <button onClick={e=>{e.stopPropagation();onEdit();}}
            className="w-6 h-6 rounded-md bg-blue-500 hover:bg-blue-600 text-white flex items-center justify-center text-xs cursor-pointer shadow">✏️</button>
          <button onClick={e=>{e.stopPropagation();onDelete();}}
            className="w-6 h-6 rounded-md bg-red-500 hover:bg-red-600 text-white flex items-center justify-center text-xs cursor-pointer shadow">🗑️</button>
        </div>
      )}
    </div>
  );
...
// ─── EDIT DEPT MODAL ──────────────────────────────────────────────────────────
const DEPT_COLORS=[
  {id:"sky",    bg:"bg-sky-600",    light:"bg-sky-50",    border:"border-sky-200",    text:"text-sky-700",    preview:"#0284c7"},
  {id:"emerald",bg:"bg-emerald-600",light:"bg-emerald-50",border:"border-emerald-200",text:"text-emerald-700",preview:"#059669"},
  {id:"violet", bg:"bg-violet-600", light:"bg-violet-50", border:"border-violet-200", text:"text-violet-700", preview:"#7c3aed"},
  {id:"rose",   bg:"bg-rose-600",   light:"bg-rose-50",   border:"border-rose-200",   text:"text-rose-700",   preview:"#e11d48"},
  {id:"orange", bg:"bg-orange-600", light:"bg-orange-50", border:"border-orange-200", text:"text-orange-700", preview:"#ea580c"},
  {id:"teal",   bg:"bg-teal-600",   light:"bg-teal-50",   border:"border-teal-200",   text:"text-teal-700",   preview:"#0d9488"},
  {id:"indigo", bg:"bg-indigo-600", light:"bg-indigo-50", border:"border-indigo-200", text:"text-indigo-700", preview:"#4f46e5"},
  {id:"amber",  bg:"bg-amber-600",  light:"bg-amber-50",  border:"border-amber-200",  text:"text-amber-700",  preview:"#d97706"},
  {id:"pink",   bg:"bg-pink-600",   light:"bg-pink-50",   border:"border-pink-200",   text:"text-pink-700",   preview:"#db2777"},
  {id:"cyan",   bg:"bg-cyan-600",   light:"bg-cyan-50",   border:"border-cyan-200",   text:"text-cyan-700",   preview:"#0891b2"},
];
const DEPT_ICONS=["⚙️","💼","👥","📣","🔧","🛒","💻","✅","📈","🎯","🏭","🚀","📋","🏪","🔑","📊","🎪","🏆","💡","🔍"];

function EditDeptModal({dept,lang,onSave,onClose,isNew}){
  const [nameAr,setNameAr]=useState(dept?.label?.ar||"");
  const [nameEn,setNameEn]=useState(dept?.label?.en||"");
  const [icon,setIcon]=useState(dept?.icon||"⚙️");
  const [colorId,setColorId]=useState(()=>{
    if(!dept?.color)return"sky";
    return DEPT_COLORS.find(c=>c.bg===dept.color.bg)?.id||"sky";
  });
  const selectedColor=DEPT_COLORS.find(c=>c.id===colorId)||DEPT_COLORS[0];
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
      <div dir="rtl" className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
        <div className="bg-slate-800 text-white px-5 py-4 flex items-center justify-between">
          <div className="font-black text-sm">{isNew?(lang==="ar"?"➕ إضافة قسم رئيسي جديد":"➕ Add New Department"):(lang==="ar"?"✏️ تعديل القسم الرئيسي":"✏️ Edit Department")}</div>
        ...
        <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-bold text-slate-600 block mb-1.5">📝 الاسم بالعربي</label>
            ...
            <div>
              <label className="text-xs font-bold text-slate-600 block mb-1.5">📝 English Name</label>
          ...
          <div>
            <label className="text-xs font-bold text-slate-600 block mb-2">🎨 اختر أيقونة</label>
            <div className="flex flex-wrap gap-2">
              {DEPT_ICONS.map(ic=>(
                <button key={ic} onClick={()=>setIcon(ic)}
                  className={`w-9 h-9 rounded-xl text-xl flex items-center justify-center cursor-pointer transition-all
                    ${icon===ic?"bg-slate-800 shadow-md scale-110":"bg-slate-100 hover:bg-slate-200"}`}>
                  {ic}
                </button>
              ))}
            </div>
          ...
          <div>
            <label className="text-xs font-bold text-slate-600 block mb-2">🎨 اختر لون القسم</label>
            <div className="flex flex-wrap gap-2">
              {DEPT_COLORS.map(c=>(
                <button key={c.id} onClick={()=>setColorId(c.id)}
                  style={{backgroundColor:c.preview}}
                  className={`w-8 h-8 rounded-xl cursor-pointer transition-all ${colorId===c.id?"ring-4 ring-offset-2 ring-slate-400 scale-110":""}`}/>
              ))}
            </div>
          </div>
          {/* Preview */}
          <div className="bg-slate-50 rounded-xl p-3">
            <div className="text-xs text-slate-500 mb-2">{lang==="ar"?"معاينة:":"Preview:"}</div>
            <div className={`inline-flex flex-col items-center gap-1.5 rounded-2xl px-4 py-3 border-2 ${selectedColor.bg} text-white`}>
              <span className="text-2xl">{icon}</span>
              <span className="text-xs font-bold">{lang==="ar"?nameAr||"اسم القسم":nameEn||"Department"}</span>
            </div>
          </div>
        </div>
        <div className="px-5 pb-5 flex gap-3 justify-end border-t border-slate-100 pt-4">
          ...
          <button onClick={()=>{
            ...
            onSave({label:{ar:nameAr||nameEn,en:nameEn||nameAr},icon,color:selectedColor});
...
// ─── SUMMARY ──────────────────────────────────────────────────────────────────
function SummaryPage({mode,scores,notes,storeInfo,recommendations,setRecommendations,grandTotal,grandMax,grandPct,criticalFails,answeredAll,totalItems,onBack,lang,t,singleDept,companyInfo}){
  ...
  return(<div dir={t.dir} className="min-h-screen bg-slate-100 print:bg-white">
    {/* Screen header */}
    <div className="bg-slate-900 text-white py-4 px-4 print:hidden">
      <div className="max-w-5xl mx-auto flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-lg font-black">📊 {t.reportTitle}</h1>
          <p className="text-slate-400 text-xs mt-0.5">{headerLabel}</p>
        </div>
        <div className="flex gap-2">
    ...
    <div className="max-w-5xl mx-auto px-4 py-6 space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm print:shadow-none print:border-slate-300">
        <div className="flex items-start justify-between gap-4 mb-5 pb-5 border-b border-slate-100">
          ...
          <div className="flex items-center gap-4">
            {co.logo?(
              <img src={co.logo} alt="logo" className="w-16 h-16 object-contain rounded-xl border border-slate-200"/>
            ):(
              <div className="w-16 h-16 bg-slate-800 rounded-xl flex items-center justify-center text-white text-2xl font-black shrink-0">
                {co.name?co.name.charAt(0):"R"}
              </div>
            )}
            <div>
              <div className="text-xl font-black text-slate-800">{co.name||"Retail Audit Pro"}</div>
              {co.nameEn&&<div className="text-sm text-slate-500" dir="ltr">{co.nameEn}</div>}
              {co.address&&<div className="text-xs text-slate-400 mt-0.5">{co.address}</div>}
              {co.phone&&<div className="text-xs text-slate-400" dir="ltr">{co.phone}</div>}
            </div>
          </div>
          {/* Report title block */}
          <div className="text-left" dir="ltr">
            <div className="text-xs text-slate-400 uppercase tracking-widest">Audit Report</div>
            <div className="text-2xl font-black text-slate-800 mt-0.5">{headerLabel}</div>
            <div className="text-xs text-slate-400 mt-1">
              {lang==="ar"?"تاريخ التقرير:":"Report Date:"} {new Date().toLocaleDateString(lang==="ar"?"ar-SA":"en-GB")}
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            [lang==="ar"?"اسم المعرض / الجهة":"Store / Entity", storeInfo.name],
            [lang==="ar"?"الرقم / الكود":"Code", storeInfo.code],
            [lang==="ar"?"المنطقة":"Region", storeInfo.region],
            [lang==="ar"?"اسم المدقق":"Auditor", storeInfo.auditor],
            [lang==="ar"?"تاريخ التدقيق":"Audit Date", storeInfo.date],
            [lang==="ar"?"نوع التدقيق":"Audit Type", headerLabel],
            [lang==="ar"?"البنود المُقيَّمة":"Items Assessed", `${answeredAll} / ${totalItems}`],
            [lang==="ar"?"بنود حرجة فاشلة":"Critical Fails", criticalFails.length],
          ].map(([label,val])=>val?(
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <div className="text-slate-400 text-sm mt-1">{grandTotal} {t.deptOf} {grandMax} {t.scoreOf}</div>
          </div>
          ...
          {DEPTS.map(d=>d.sections.map(s=>{const sp=secPct(s,scores),barC=sp>=80?"#10b981":sp>=50?"#f59e0b":"#ef4444";return(
            <div key={s.id} className="flex items-center gap-3">
              ...
              <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                <div className="h-full rounded-full" style={{width:`${sp}%`,backgroundColor:barC}}/>
              </div>
      ...
      {/* ── SIGNATURES ── */}
      <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
        <h2 className="font-bold text-slate-700 mb-5 text-sm">{lang==="ar"?"التوقيعات والاعتماد":"Signatures & Approval"}</h2>
        <div className="grid grid-cols-3 gap-6 text-center">
          {sigs.map(title=>(
            <div key={title}>
              <div className="text-xs text-slate-500 mb-1 font-medium">{title}</div>
              <div className="border-b-2 border-slate-300 pt-14 mb-1"></div>
              <div className="text-xs text-slate-400">{t.sigLine}</div>
            </div>
          ))}
        </div>
      </div>

      {/* ── POWERED BY ABS FOOTER ── */}
      <div className="bg-slate-800 rounded-2xl p-5 text-center">
        <div className="flex items-center justify-center gap-3 flex-wrap">
          <div className="text-slate-400 text-xs">{lang==="ar"?"تم إعداد هذا التقرير بواسطة":"This report was prepared by"}</div>
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 bg-amber-500 rounded-lg flex items-center justify-center text-white text-sm font-black">A</div>
            <div className="text-right">
              <div className="text-white text-xs font-black">ABS — Advanced Business Services</div>
              <div className="text-slate-400 text-[10px]">خدمات الأعمال المتقدمة</div>
            </div>
          </div>
        </div>
        <div className="text-slate-600 text-[10px] mt-2">
          Powered by ABS Advanced Business Services © {new Date().getFullYear()} — جميع الحقوق محفوظة
        </div>
      </div>
...
// ─── MODE SELECTOR ────────────────────────────────────────────────────────────
function ModeSelector({lang,setLang,t,onSelect,companyInfo,setCompanyInfo}){
  const handleLogo=(e)=>{
    const file=e.target.files[0];
    if(!file)return;
    const reader=new FileReader();
    reader.onload=ev=>setCompanyInfo(p=>({...p,logo:ev.target.result}));
    reader.readAsDataURL(file);
  };
  return(
    <div dir={t.dir} className="min-h-screen bg-slate-900 flex flex-col items-center justify-center px-4 py-8">
      ...
      {/* Company Info Card */}
      <div className="w-full max-w-2xl bg-white/5 border border-white/10 rounded-2xl p-5 mb-5">
        <div className="font-bold text-white text-sm mb-4 flex items-center gap-2">
          <span>🏢</span>
          <span>{lang==="ar"?"بيانات الشركة (تظهر في التقارير)":"Company Info (appears in reports)"}</span>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-[10px] text-slate-400 block mb-1">{lang==="ar"?"اسم الشركة بالعربي":"Company Name (Arabic)"}</label>
            <input value={companyInfo.name} onChange={e=>setCompanyInfo(p=>({...p,name:e.target.value}))}
              className="w-full bg-white/10 border border-white/10 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-amber-400"
              placeholder={lang==="ar"?"مثال: شركة النجم للتجزئة":"e.g. Al-Najm Retail Co."}/>
          </div>
          <div>
            <label className="text-[10px] text-slate-400 block mb-1">Company Name (English)</label>
            <input value={companyInfo.nameEn} onChange={e=>setCompanyInfo(p=>({...p,nameEn:e.target.value}))} dir="ltr"
              className="w-full bg-white/10 border border-white/10 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-amber-400"
              placeholder="e.g. Al-Najm Retail Co."/>
          </div>
          <div>
            <label className="text-[10px] text-slate-400 block mb-1">{lang==="ar"?"العنوان":"Address"}</label>
            <input value={companyInfo.address} onChange={e=>setCompanyInfo(p=>({...p,address:e.target.value}))}
              className="w-full bg-white/10 border border-white/10 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-amber-400"
              placeholder={lang==="ar"?"الرياض، المملكة العربية السعودية":"Riyadh, Saudi Arabia"}/>
          </div>
          <div>
            <label className="text-[10px] text-slate-400 block mb-1">{lang==="ar"?"رقم الهاتف":"Phone"}</label>
            <input value={companyInfo.phone} onChange={e=>setCompanyInfo(p=>({...p,phone:e.target.value}))} dir="ltr"
              className="w-full bg-white/10 border border-white/10 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-amber-400"
        ...
        <div className="mt-3 flex items-center gap-3">
          <label className="cursor-pointer flex items-center gap-2 bg-white/10 hover:bg-white/20 border border-white/10 rounded-xl px-4 py-2 text-white text-xs font-bold transition-colors">
            <span>🖼️</span>
            <span>{lang==="ar"?"رفع شعار الشركة":"Upload Logo"}</span>
            <input type="file" accept="image/*" onChange={handleLogo} className="hidden"/>
          </label>
          {companyInfo.logo&&(
            <div className="flex items-center gap-2">
              <img src={companyInfo.logo} alt="logo" className="w-10 h-10 object-contain rounded-lg border border-white/20 bg-white/10"/>
              <button onClick={()=>setCompanyInfo(p=>({...p,logo:""}))} className="text-slate-400 hover:text-red-400 text-xs cursor-pointer">✕</button>
            </div>
          )}
          {!companyInfo.logo&&<span className="text-slate-500 text-xs">{lang==="ar"?"لم يتم رفع شعار":"No logo uploaded"}</span>}
        </div>
      </div>
...
// ─── HQ DEPT PICKER ───────────────────────────────────────────────────────────
function HQDeptSelector({lang,setLang,t,hqData,hqDepts,onSelect,onBack,onEditDepts}){
  ...
  return(
    <div dir={t.dir} className="min-h-screen bg-slate-900">
      {deptModal?.type==="editDept"&&<EditDeptModal dept={deptModal.dept} lang={lang} isNew={false}
        onSave={data=>{onEditDepts(prev=>prev.map(d=>d.id!==deptModal.dept.id?d:{...d,...data}));setDeptModal(null);}}
        onClose={()=>setDeptModal(null)}/>}
      {deptModal?.type==="addDept"&&<EditDeptModal dept={null} lang={lang} isNew={true}
        onSave={data=>{onEditDepts(prev=>[...prev,{id:genId("hqdept"),sections:[],...data}]);setDeptModal(null);}}
        onClose={()=>setDeptModal(null)}/>}
      {confirmModal&&<ConfirmModal lang={lang} message={confirmModal.message}
            ...
            return(
              <div key={d.id} className="relative group">
                <button onClick={()=>onSelect(d.id)}
                  className={`w-full relative bg-white rounded-2xl border-2 p-5 text-center cursor-pointer hover:shadow-xl hover:scale-105 transition-all duration-200
                ...
                <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={e=>{e.stopPropagation();setDeptModal({type:"editDept",dept:d});}}
                    className="w-7 h-7 rounded-lg bg-blue-500 hover:bg-blue-600 text-white flex items-center justify-center text-xs cursor-pointer shadow-lg">✏️</button>
                  <button onClick={e=>{e.stopPropagation();setConfirmModal({
                    message:lang==="ar"?`هل تريد حذف قسم "${d.label.ar}"؟`:`Delete department "${d.label.en}"?`,
                    onConfirm:()=>{onEditDepts(prev=>prev.filter(x=>x.id!==d.id));setConfirmModal(null);}
                  });}}
                    className="w-7 h-7 rounded-lg bg-red-500 hover:bg-red-600 text-white flex items-center justify-center text-xs cursor-pointer shadow-lg">🗑️</button>
                </div>
              </div>
            );
          })}
          {/* Add new dept card */}
          <button onClick={()=>setDeptModal({type:"addDept"})}
            className="bg-white/5 hover:bg-white/10 border-2 border-dashed border-white/20 hover:border-amber-400 rounded-2xl p-5 text-center cursor-pointer transition-all duration-200 flex flex-col items-center justify-center gap-2 min-h-[180px]">
            <span className="text-4xl">➕</span>
            <span className="text-sm font-bold text-slate-400">{lang==="ar"?"إضافة قسم جديد":"Add Department"}</span>
          </button>
        </div>
      </div>
    </div>
...
function AuditView({mode,dept,allDepts,scores,setScores,notes,setNotes,openSections,setOpenSections,storeInfo,setStoreInfo,lang,setLang,t,onBack,onReport,onEditDepts}){
  ...
  return(
    <div dir={t.dir} className="min-h-screen bg-slate-100">
      {/* MODALS */}
      {modal?.type==="editDept"&&<EditDeptModal dept={modal.dept} lang={lang} isNew={false}
        onSave={data=>{onEditDepts(prev=>prev.map(d=>d.id!==modal.dept.id?d:{...d,...data}));setModal(null);}}
        onClose={()=>setModal(null)}/>}
      {modal?.type==="addDept"&&<EditDeptModal dept={null} lang={lang} isNew={true}
        onSave={data=>{
          const newDept={id:genId("dept"),sections:[],...data};
          onEditDepts(prev=>[...prev,newDept]);
      ...
      {mode==="store"&&(
        <div className="max-w-5xl mx-auto px-4 py-4">
          <div className="flex gap-3 overflow-x-auto pb-1 items-start">
            {allDepts.map(d=>(
              <DeptCard key={d.id} dept={d} scores={scores} active={activeDeptId===d.id}
                onClick={()=>setActiveDeptId(d.id)} lang={lang}
                editMode={editMode}
                onEdit={()=>setModal({type:"editDept",dept:d})}
                onDelete={()=>setModal({type:"confirmDel",
                  message:lang==="ar"?`هل تريد حذف قسم "${d.label.ar}" وجميع بنوده؟`:`Delete department "${d.label.en}" and all its items?`,
                  onConfirm:()=>{
                    onEditDepts(prev=>prev.filter(x=>x.id!==d.id));
                    if(activeDeptId===d.id)setActiveDeptId(allDepts.find(x=>x.id!==d.id)?.id||null);
                    setModal(null);
                  }})}/>
            ))}
            {editMode&&(
              <button onClick={()=>setModal({type:"addDept"})}
                className="flex flex-col items-center gap-1.5 rounded-2xl px-4 py-3 border-2 border-dashed border-amber-300 bg-amber-50 text-amber-600 hover:bg-amber-100 transition-all min-w-[110px] cursor-pointer shrink-0">
                <span className="text-2xl">➕</span>
                <span className="text-xs font-bold text-center">{lang==="ar"?"قسم جديد":"New Dept"}</span>
              </button>
            )}
          </div>
        </div>
...
export default function RetailAuditProV2(){
  ...
  // Company info (shared across all reports)
  const [companyInfo,setCompanyInfo]=useState({name:"",nameEn:"",address:"",phone:"",logo:""});
  ...
  if(!mode) return <ModeSelector lang={lang} setLang={setLang} t={t} onSelect={m=>{setMode(m);setShowSummary(false);}} companyInfo={companyInfo} setCompanyInfo={setCompanyInfo}/>;
  if(mode==="hq"&&!hqDeptId) return <HQDeptSelector lang={lang} setLang={setLang} t={t} hqData={hqData} hqDepts={hqDepts} onSelect={id=>{setHqDeptId(id);setShowSummary(false);}} onBack={()=>setMode(null)} onEditDepts={handleHqDeptsEdit}/>;
